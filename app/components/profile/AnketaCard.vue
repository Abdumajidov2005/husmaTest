  ...
  <article class="card border-l-2" :class="meta.border">
    ...
    <div class="flex items-center justify-between gap-2 mb-3">
      <span
        ...
        :class="meta.badge"
      >
        <span class="w-1.5 h-1.5 rounded-full" :class="meta.dot" aria-hidden="true" />
        {{ meta.label }}
      </span>
    ...
    <!-- Bekor qilish (ikki bosqichli tasdiq) -->
    <div class="pt-1">
      <Transition name="fade" mode="out-in">
        <div v-if="!confirming" key="action" class="flex justify-end">
          ...
          >
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 16 16" aria-hidden="true">
              <path d="M3 4h10M6.5 4V3a1 1 0 011-1h1a1 1 0 011 1v1M4 4l.5 9a1 1 0 001 1h5a1 1 0 001-1L12 4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            {{ t('anketa.cancel') }}
          </button>
        </div>
        ...
        >
          <span class="text-xs text-red-600 dark:text-red-400">{{ t('anketa.confirm') }}</span>
...
watch(() => confirming.value, (v) => v && haptic('light'))
</script>
