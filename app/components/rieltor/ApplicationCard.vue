<template>
  <article class="card border-l-2" :class="statusMeta.border">
    <!-- Header -->
    <div class="flex items-center gap-2.5 mb-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">
          {{ application.name || t("rieltor.unknownName") }}
        </p>
        <time
        ...
        >
          ...
          <span class="text-xs text-brand-600 dark:text-brand-400 font-medium">
            {{ application.vaqtOldin || timeAgo(application.createdAt) }}
          </span>
        </time>
      </div>
    ...
    <div class="flex gap-2">
      ...
      <UiButton
        ...
        variant="secondary"
        size="md"
        custom-class="flex-1 !rounded-2xl !text-sm font-semibold"
        :aria-label="t('rieltor.callAria', { name: application.name })"
      >
        <svg class="w-4 h-4" fill="none" viewBox="0 0 20 20" aria-hidden="true">
          <path
            d="M2 3.5C2 2.67 2.67 2 3.5 2h2.1c.72 0 1.35.5 1.47 1.2l.6 3a1.5 1.5 0 01-.84 1.6l-.9.45a10.52 10.52 0 005.32 5.32l.45-.9a1.5 1.5 0 011.6-.84l3 .6c.7.12 1.2.75 1.2 1.47v2.1c0 .83-.67 1.5-1.5 1.5A16.5 16.5 0 012 3.5z"
            stroke="currentColor"
            stroke-width="1.5"
            stroke-linejoin="round"
          />
      ...
      <!-- Yopish -->
      <UiButton
        v-else-if="application.status === 'in_progress'"
        type="button"
        variant="primary"
        size="md"
        haptic="medium"
...
async function onAccept() {
  ...
  try {
    emit("accept", props.application.id);
  } finally {
...
async function onClose() {
  ...
  try {
    emit("close", props.application.id);
  } finally {
