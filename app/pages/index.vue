    ...
    <main>
      <Transition :name="transitionName" mode="out-in">
        ...
        <div v-if="showForm" key="form">
          <div v-if="!supportsBackButton" class="flex items-center gap-2 px-3 pt-3">
            <UiButton
              variant="ghost"
              size="sm"
              haptic="light"
            ...
            <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ t('common.back') }}</span>
          </div>
        ...
        <RieltorTab v-else-if="activeTab === 'rieltor' && isRieltor" key="rieltor" />
      </Transition>
    </main>
...
const { t } = useI18n()
const {
  ready, haptic, notificationHaptic,
  supportsBackButton, showBackButton, hideBackButton, onBackButton, offBackButton,
} = useTelegram()
...
watch(showForm, (open) => {
  ...
  if (open) {
    formBackCb = () => { showForm.value = false }
...
function handleFormSubmit(data) {
  addApplication(data, ownerId.value)
