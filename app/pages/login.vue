    ...
    <div class="w-full max-w-sm card shadow-sm">
      ...
      <template v-if="step === 'otp'">
        <div class="flex items-center gap-3 mb-5">
          <button
            type="button"
            class="!w-auto flex items-center justify-center w-8 h-8 rounded-lg text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
          ...
          <div>
            <h2 class="text-base font-bold text-gray-900 dark:text-white">{{ t('login.otpTitle') }}</h2>
        ...
        <div class="flex justify-center mb-4">
          <div class="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center">
            <svg class="w-8 h-8 text-blue-500" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.248l-2.04 9.613c-.152.674-.553.838-1.12.521l-3.1-2.285-1.496 1.44c-.165.165-.304.304-.624.304l.223-3.165 5.754-5.198c.25-.222-.054-.346-.388-.124l-7.111 4.48-3.063-.957c-.666-.208-.68-.666.14-.986l11.963-4.614c.555-.2 1.04.136.862.971z"/>
            </svg>
          </div>
        </div>

        <div class="mb-4 flex items-start gap-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl px-3 py-2.5">
          <svg class="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 16 16">
        ...
        <form novalidate class="space-y-3" @submit.prevent="handleOtpSubmit">
          ...
          <UiButton type="submit" variant="primary" size="md" haptic="medium" block :loading="isLoading" class="mt-2">
            {{ t('login.otpConfirm') }}
          </UiButton>
        </form>
      ...
      <template v-else>
        ...
        <form novalidate class="space-y-3" @submit.prevent="handleSubmit">
          ...
          <!-- Viloyat -->
          <UiFormField v-if="isRegister" :label="t('form.cityLabel')" icon="city" :error="errors.city">
            <select
              v-model="form.city"
          ...
          <div v-if="isRegister && form.city">
            ...
            <div
              ...
              :class="{ 'border-red-400 dark:border-red-500': errors.districts }"
            >
              <label
              ...
              >
                ...
                />
                <span class="text-sm text-gray-800 dark:text-gray-200">{{ d.nomi }}</span>
              </label>
            </div>
            <p v-if="errors.districts" class="text-xs text-red-500 mt-1">{{ errors.districts }}</p>
          </div>
...
const form         = reactive({ name: '', phone: '', login: '', password: '', city: '', districts: [] })
...
function validate() {
  ...
  if (isRegister.value) {
    ...
    if (!form.districts.length) errors.districts = t('login.errDistrictsRequired')
  }
